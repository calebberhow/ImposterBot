...
async function start(client: ServiceClient, interaction: ChatInputCommandInteraction)
{
  ...
  let stream = await play.stream("https://www.youtube.com/watch?v=yXQViqx6GMY");
...
{
  return `mariah_stop_${guild_id}`;
}

const builder = new SlashCommandBuilder()
  .setName('mariah')
  .setDescription('starts/stops mariah carey.')
  .addSubcommand(cmd => cmd
    .setName('start')
    .setDescription('starts mariah on the given channel.')
  ...
  .addSubcommand(cmd => cmd
    .setName('stop')
    .setDescription('stops ImposterBot from playing mariah carey.'))
